...
class InitialDownsample(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(InitialDownsample, self).__init__()
        self.downsample = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(num_features=out_channels),#对batch(20)的每个特征通道做归一化
            nn.ReLU(),
        )
...
class SRNet(nn.Module):
    def __init__(self, data_format='NCHW', init_weights=True):
        ...
        # 第一种结构类型
        self.initial_downsample = InitialDownsample(1, 64)
        self.layer1 = Block1(64, 64)
        self.layer2 = Block1(64, 16)
    ...
    def forward(self, inputs):
        ...
        # 第一种结构类型
        x = self.initial_downsample(self.inputs)
        x = self.layer1(x)
        x = self.layer2(x)
