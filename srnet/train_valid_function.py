...
def validation(model, valid_loader, device, criterion, writer, train_step):
    ...
    with torch.no_grad():
        for images, labels in valid_loader:
            ...
            loss = criterion(model_output, labels)# [[0.3,0.7]] [1]
...
def train(model,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for index, (images, labels) in enumerate(train_loader):
            ...
            if train_step % write_interval == 0:
                ...
                print('EPOCH: %d/%d || currIter: %d || train_loss: %.6f || train_acc: %.6f' %
                      (epoch, EPOCHS, train_step, total_loss, temp_acc))

                writer.add_scalar('train_accuracy', temp_acc, train_step)
                writer.add_scalar('train_loss', total_loss, train_step)
                total_loss = 0
                total_acc = 0

            if train_step % save_interval == 0:
                # 保存模型
                torch.save(model.state_dict(), './HUGO_01_Model/weight3/Model_' + str(train_step) + '.pth')
                model_save_cnt += 1
        ...
        print('EPOCH: %d || loss: %.6f' % (epoch, tmp_loss / 350))
    writer.close()
...
def test(model, test_loader, device, weight_path=None):
    ...
    with torch.no_grad():
        for index, (images, labels) in enumerate(test_loader):
            step_cnt += 1
            cover_img, stego_img = images.view(-1, 1024, 1024, 1).to(device)
